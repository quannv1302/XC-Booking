...
const MENU_ITEMS = [
  ...
  { id: 'bookings', label: 'Quản lý Kế hoạch', icon: FileText },
  { id: 'jobs', label: 'Quản lý Yêu cầu', icon: Briefcase },
  { id: 'journeys', label: 'Quản lý Hành trình', icon: Map },
];

const Sidebar: React.FC<SidebarProps> = ({ collapsed, setCollapsed, activeModule, onChangeModule }) => {
  return (
    <aside 
      className={`
...
const NavItem: React.FC<NavItemProps> = ({ icon: Icon, label, active, collapsed, onClick }) => {
  return (
    <div 
      ...
      `}
    >
      <Icon size={20} className={active ? 'text-brand-green' : 'text-green-200 group-hover:text-white'} />
