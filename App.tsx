...
import { Search, Plus, Filter, Download, ChevronDown, Briefcase, Map, Layers, CheckCircle2, Clock } from 'lucide-react';
      ...
      items: [
        {
          ...
          packingListFile: 'PL_DoGiaDung.xlsx'
        },
        {
          ...
          packingListFile: 'PL_VaiCuon.pdf'
        },
        {
          name: 'Linh kiện nhỏ',
          type: 'Điện tử',
    ...
    jobs: [
      {
        ...
        performDate: '2023-10-22T14:30',
        note: 'Sang tải 3000 chiếc đồ gia dụng. Cẩn thận hàng dễ vỡ.',
        supplementalReqs: ['Sang tải sau 22h', 'Hàng quá khổ'],
        otherReqContent: ''
      },
      {
        id: 'j2',
        jobCode: 'REQ-2',
        type: 'transshipment',
        transshipmentMethod: 'manual',
        status: 'pending',
        vehicleCNId: 'cn2',
        vehicleVNId: 'vn1',
        vehicleCNPlate: '12C-999.88',
        vehicleVNPlate: '29H-123.45',
        cargoName: 'Linh kiện nhỏ',
        quantity: '200 Kg',
        packingSpec: 'Bao tải',
        performDate: '2023-10-22T16:00',
        note: '',
        supplementalReqs: ['Khác'],
        otherReqContent: 'Cần bọc lót kỹ'
      }
    ]
  },
  '2': {
    id: '2',
    bookingNumber: 'BK-2023-002',
    type: 'Xuất khẩu',
    licensePlate: 'Multi',
    nature: 'Nông sản',
    status: 'received',
    csInCharge: 'Trần Thị C',
    createdDate: '21/10/2023 09:00',
    customerId: 'CUST-EX-9912',
    borderGate: 'Tân Thanh',
    importExportType: 'Tiểu ngạch',
    needsCustomsClearance: false,
    needsYardService: true,
    etaGeneral: '2023-10-23T07:00',
    fieldOps: 'Lê Văn E',
    fieldOpsPhone: '0901.222.333',
    customsOps: '',
    customsOpsPhone: '',
    generalNotes: 'Hàng cần bảo quản mát',
    vehiclesCN: [],
    vehiclesVN: [],
    cargo: {
      mode: 'bulk',
      items: [{ name: 'Thanh long', quantity: '20 Tấn', packingSpec: 'Thùng xốp' }]
    },
    jobs: []
  }
};
...
const App: React.FC = () => {
  ...
  const [currentView, setCurrentView] = useState<'list' | 'detail' | 'create' | 'edit' | 'jobDetail'>('list');
  const [selectedBookingId, setSelectedBookingId] = useState<string | null>(null);

  // State for Job Detail
  const [selectedJob, setSelectedJob] = useState<JobWithBooking | null>(null);
  // New state to control which tab is active when opening BookingDetail
  const [detailTab, setDetailTab] = useState<'general' | 'vehicles' | 'cargo' | 'jobs'>('general');
  ...
  const handleModuleChange = (module: string) => {
    ...
    if (module === 'jobs') {
      // When switching to jobs module, show the list by default
      setCurrentView('list');
    }
  };
  ...
  const handleViewDetail = (id: string) => {
    ...
    setDetailTab('general'); // Default tab
  ...
  const handleViewBookingFromJob = () => {
    if (selectedJob) {
      handleViewDetail(selectedJob.bookingId);
    }
  };
  ...
  // Flatten Jobs Data for the Jobs Module
              ...
              <>
                ...
                {currentView === 'jobDetail' && selectedJob && (
                  <div className="p-4 md:p-6">
                    <JobDetail
                      job={selectedJob}
                      booking={MOCK_DB[selectedJob.bookingId]}
                      onBack={handleBackToList}
                      onViewBooking={handleViewBookingFromJob}
                    />
                  </div>
                )}
              </>
